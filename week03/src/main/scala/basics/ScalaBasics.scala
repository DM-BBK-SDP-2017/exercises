...
object ScalaBasics {
  ...
  if (r.length != 1) {
  if (r.head > minRecursive(r.tail)) {return minRecursive(r.tail)}
  }
  return r.head;

      /*
    case r if r.length == 1 => r.head
    case r if r.head > minRecursive(r.tail) => minRecursive(r.tail)
    case r => r.head
     */
  }
  /**
  ...
  def isPalindrome(s: String): Boolean = {
    ...
    /*
    val r = s.replaceAll("?|.|,|;|!|-| ", "").toLowerCase
    return r == r.reverse
*/
  }
