...
class FunctionsTestSuite extends FunSuite {
  ...
  test("Tail deals with Nil list") {
    assertThrows[IllegalArgumentException] {tail(Nil)}
  }

  test("Tail returns Nil on one element list") {
  ...
  test("setHead accepts Nil list with element") {
    assert(setHead(List(Nil), "A") == List("A"))
  }

  test("Tail deals with Null element") {
    assertThrows[IllegalArgumentException] {setHead(List(1,2,3), Unit)}
  }
  ...
  val testList = Range(1,100).toList

  test("init removes the last element from longer list") {
    assert(!init(testList).contains(99));
  }

  test("init doesn't remove other elements from list") {
    assert(init(testList).contains(1) &&
      init(testList).contains(2) &&
      init(testList).contains(97) &&
      init(testList).contains(98))

  }

  test("init throws error on Nil list ") {
