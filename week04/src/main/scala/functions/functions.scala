...
object Funcs {
  ...
  /**
    ...
    *         element of ls.
    **/
  def setHead[A](ls: List[A], a: A): List[A] = (ls, a) match {
    case (_, Unit) => throw new IllegalArgumentException
    case (ls,a) => a :: tail(ls)
  }
   ...
   */
  def foldLeft[A, B](ls: List[A], z: B)(f: (B, A) => B): B = (ls, z, f) match {
    case (Nil, z, f) => z
    case (ls, z, f) => foldLeft(ls.tail, f(z, ls.head))(f)
  ...
  def sum(ls: List[Double]): Double = {
    ...
    foldLeft(ls, 0.0)((double: Double, double2: Double) => double + double2)
  }

  def product(ls: List[Double]): Double = {
    foldLeft(ls, 1.0)((double: Double, double2: Double) => double * double2)

  }

  def length[A](ls: List[A]): Int = {
    foldLeft(ls, 0)((i: Int, j: A) => i + 1)

  }

  def reverse[A](ls: List[A]): List[A] = {
    foldLeft(ls, Nil: List[A])((newList: List[A], a: A) => a :: newList)
  }

  def flatten[A](ls: List[List[A]]): List[A] = {
    foldLeft(ls, Nil: List[A])((aList: List[A], bList: List[A]) => aList ++ bList)
  }
  ...
  def map[A, B](ls: List[A])(f: A => B): List[B] = ls match {
    case (Nil) => Nil
    case (ls) => f(ls.head) :: map(ls.tail)(f)
  ...
  /**
    ...
    */
  def filter[A](ls: List[A])(f: A => Boolean): List[A] = (ls, f) match {
    case (Nil, _) => Nil
    case (ls, f) if f(ls.head) == true => ls.head :: filter(ls.tail)(f)
    case _ => filter(ls.tail)(f)
  }
  ...
  /**
    ...
    *         elements of ls.
    */
  def flatMap[A, B](ls: List[A])(f: A => List[B]): List[B] = (ls, f) match {
    case (Nil, _) => List.empty[B]
    case (ls: List[A], f) => f(ls.head) ++ flatMap(ls.tail)(f)
  }
  ...
  /**
    ...
    */
  //def maxAverage(ls: List[(Double, Double)]): Double = foldLeft((map(ls)(a => List(a._1, a._2).max)),0.0)((y, x) =>  )
  def maxAverage(ls: List[(Double, Double)]): Double = sum(map(ls)(a => List(a._1, a._2).max)) / ls.size
  ...
  def variance(ls: List[Double]): Double = {
    ...
    def mean(ls2: List[Double]): Double = { sum(ls2) / ls2.size }

    val M = mean(ls) // MEAN

    mean(map(ls)(V => scala.math.pow(V - M, 2)))


    // (List of Double means, accumulator)((acc, head) => double): Double
